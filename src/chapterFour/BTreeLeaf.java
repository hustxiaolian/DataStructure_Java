...
public class BTreeLeaf<K extends Comparable<? super K>> extends AbstractBTreeNode<K> {
	...
	protected K splitSelf(AbstractBTreeNode<K> newNode) {
		...
		BTreeLeaf<K> node = (BTreeLeaf<K>)newNode;
		int i = 0;
		while(i < degree - 1) {
	...
	protected void merge(K middle, AbstractBTreeNode<K> sibling) {
		...
		BTreeLeaf<K> node = (BTreeLeaf<K>)sibling;
